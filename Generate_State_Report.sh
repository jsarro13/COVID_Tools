...
cp 1623249503732.metadata.tsv metadata.working.tsv
##########******************************************
...
####Change upload file
awk -F ',' '{print $6","$27}' 210601_CovidSeqRun-06_EpiCoV_BulkUpload.csv > date.tmp.csv
#####*****************
sort -t , -k2 date.tmp.csv > date.sorted.csv
rm date.tmp.csv

#########edit date in output file for each run############
echo "Barcode,Lineage,GISAID_ID,Clade,Date,Pangloin_Version" >  CovidSeqRun-06_06-3-2021.state.csv
join -t , -2 2 state.merge.2.csv date.sorted.csv >>  CovidSeqRun-06_06-3-2021.state.csv
rm state.merge.2.csv
rm date.sorted.csv
sed -i -e '2,$s/$/,v.3.0.3/g' "CovidSeqRun-06_06-3-2021.state.csv"
